...
gastos_mappings = {
    'CODIGO': {
        ...
        '09150': 'Reparación de equipos audiovisuales, fotogrficos y de procesamiento de la información',
        ...
        '10112': 'Segundo ciclo de educacin infantil (3-6 años) (grupo 02 cned-p-2014)',
...
def apply_gastos_castings(df):
    ...
    for column in columns_to_cast:
        if column in df.columns:
            ...
            # Interpret the last two digits as decimals
            df[column] = df[column].apply(lambda x: float(x[:-2] + '.' + x[-2:]) if x.isdigit() else None)
...
# Directory containing the intermediate_outputs CSV files
directory = 'intermediate_outputs'
final_outputs_directory = 'final_outputs_enriched'

# Explicitly process each file with its corresponding mapping
files_to_process = [
    ('EPFmhogar_2023.csv', miembros_mappings),
    ('EPFgastos_2023.csv', gastos_mappings),
    ('EPFhogar_2023.csv', hogares_mappings)
]

for filename, mapping in files_to_process:
    input_file = os.path.join(directory, filename)
    output_file = os.path.join(final_outputs_directory, f"{os.path.splitext(os.path.basename(filename))[0]}.csv")
